...
const Page2: React.FC<RouteComponentProps<IProps>> = ({
  ...
  const handleFetchInfoUser = () => {
    if (inputValue) {
      getUserInfoById(+inputValue);
    }
  };

  const handleFetchPostsInfo = () => {
    getPostsInfo();
  };
  ...
  return (
    <div>
      ...
      <button onClick={handleFetchInfoUser}>
      ...
      {userInfo.id ? (
        <div>
          ...
          <button onClick={handleFetchPostsInfo}>
      ...
      {Array.isArray(postsInfo) && postsInfo.length > 0
        ...
        : userInfo.id && isLoading && <div>Загружаем данные ...</div>}
    </div>
  );
};
